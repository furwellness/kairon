...
class LanguageModelFeaturizer(DenseFeaturizer, GraphComponent, ABC):
    ...
    def _load_model_metadata(self) -> None:
        ...
        if self.model_name not in ModelFactory.models:
            raise KeyError(
                f"'{self.model_name}' not a valid model name. Choose from "
                f"{str(list(ModelFactory.models.keys()))} or create"
    ...
    def _load_model_instance(self) -> None:
        ...
        self.model_factory = ModelFactory.get_instance(
            self.model_name,
            self.model_weights,
            getenv("TRANSFORMER_CACHE", self.cache_dir),
            self.from_pt,
            getenv("LOCAL_MODEL", False),
        )
        ...
        # while feeding input.
        self.pad_token_id = self.model_factory.tokenizer.unk_token_id
        self.max_model_sequence_length = self.model_factory.MAX_SEQUENCE_LENGTHS
    ...
    def _lm_tokenize(self, text: Text) -> Tuple[List[int], List[Text]]:
        ...
        """
        split_token_ids = self.model_factory.tokenizer.encode(
            text, add_special_tokens=False
        )

        split_token_strings = self.model_factory.tokenizer.convert_ids_to_tokens(
            split_token_ids
        )
    ...
    def _add_lm_specific_special_tokens(
        ...
        augmented_tokens = [
            self.model_factory.preprocessor(example_token_ids)
    ...
    def _lm_specific_token_cleanup(
        ...
        return self.model_factory.tokens_cleaner(split_token_ids, token_strings)
    ...
    def _post_process_sequence_embeddings(
        ...
        for example_embedding in sequence_embeddings:
            ...
            ) = self.model_factory.post_processor(example_embedding)
    ...
    def _compute_batch_sequence_features(
        ...
        """
        model_outputs = self.model_factory.model(
